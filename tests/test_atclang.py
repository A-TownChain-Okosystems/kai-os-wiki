...
from atclang.vm.atcvm import ATCVM, ATCVMError, RequireError, OP, Instruction, ATCFunction
...
class TestATCLangIntegration(unittest.TestCase):
    ...
    def run_source(self, source):
        ...
        if module.functions and len(module.instructions) <= 1:
            # Contract code only — run first function
            main_instrs = list(module.functions.values())[0]
        else:
            # Top-level code (may include function definitions + calls)
            main_instrs = list(module.instructions)
            # Ensure program ends with RETURN (compiler now handles this for last expr)
